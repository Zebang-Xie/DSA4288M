...
from New_optimizer import create_custom_optimizer
...
if torch.cuda.is_available():
    ...
    device = torch.device("mps") # Mac M1/M2/M3 专用
else:
    device = torch.device("cpu")

print(f"Current implementation is running on: {device}")
...
def train_validate(
    ...
    for idx, data in enumerate(train_loader):
        ...
        if train_step > 1000:
            current_optimizer = custom_optimizer
            optimizer_step(
            _lm_loss/(args.grad_acc), current_optimizer, model, scheduler, args, is_update=is_update
        )
        else:
            current_optimizer = adam_optimizer
            optimizer_step(
            _lm_loss/(args.grad_acc), current_optimizer, model, scheduler, args, is_update=is_update
        )
...
if __name__ == '__main__':
    ...
    adam_optimizer = create_adam_optimizer_from_args(lm_net, args)
    custom_optimizer = create_custom_optimizer(lm_net, args)

    if args.max_step is None:
        args.max_step = (args.max_epoch * train_data.num_batches + args.world_size - 1) // args.world_size
        print('set max_step:', args.max_step)

    scheduler = create_optimizer_scheduler(adam_optimizer, args)
    # if args.fp16:
    ...
    try:
        ...
        for epoch in itertools.count(start=1):
            train_step = train_validate(
                lm_net, adam_optimizer,custom_optimizer, scheduler, train_loader, valid_loader, args, 
